...
exports.methods = {
    ...
    async __ca_init__() {
        ...
        if (this.state.isAdmin) {
            ...
            const fullMapName = json_rpc.joinName(this.__ca_getName__(),
                                                  'forwarding');
            ...
            const rule = this.$.security.newSimpleRule('handlePageChange',
    ...
    //     after JSON parsing
    async handlePageChange(topic, info, caller) {
        this.$.log && this.$.log.debug(
            `handlePageChange: ${topic} ${info} ${caller}`
        );
